document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch(`https://atman.onrender.com/blog/${blogId}`)
        ...
        .then(data => {
            ...
            document.getElementById('blog-timestamp').innerText = formatTimeDifferences(data.createdAt);
            document.getElementById('blog-content').innerHTML = data.content;

            document.getElementById('profile-image').src = data?.userDetails?.profile || "./assets/img/defaultpic.jpg";
            document.getElementById('profile-name').innerText = data?.userDetails?.nickname || "anonymus";
            ...
            if (data.userDetails?.linkedin) {
                document.getElementById('linkedin-link').innerHTML = `<a href="${data?.userDetails?.linkedin}" target="_blank"><i class="fa fa-linkedin"></i> LinkedIn</a>`;
            } else {
                document.getElementById('linkedin-link').innerHTML = "Anonymous";
            }

            const shareButton = document.getElementById('blog-extra').querySelector(".share-btn");
